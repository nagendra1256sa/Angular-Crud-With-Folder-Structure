...
export class Service {
  ...
  UpdateItem(data: EditDetailsType, id: number): Observable<EditDetailsType> {
    return this._http.put<EditDetailsType>(
  ...
  getItemList(): Observable<typeChek> {
    ...
    return this._http.get(api, { observe: 'response' }).pipe(
      map((response: HttpResponse<any>) => {
        ...
        if (status === HttpStatusCode?.Ok) {
          ...
          return {
            success: true,
            responseData: data.map((items: any) => this._items.adapt(items)),
          };
        } else {
          return { responseData: [] };
        }
      ...
      catchError((error: HttpErrorResponse) => {
        return of({
          success: false,
          message: this.translate.instant('ITEMS.404ERROR'),
        });
      })
    );
  }
